...
public class MutualExclusion {
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
        logger.info("KJØRER DECISION MED CONDITION: " + condition);
		...
		switch(condition) {
			...
			case 2: {
               ...
               /*  int comparisonResult;
				if(ownClock == sendingClock){
                    comparisonResult = node.getNodeID().compareTo(message.getNodeID());
                }else if (ownClock < sendingClock){
                    comparisonResult = -1;
                }else{
                    comparisonResult = 1;
                } */
